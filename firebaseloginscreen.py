...
class FirebaseLoginScreen(Screen, EventDispatcher):
    ...
    def sign_up(self, email, password,shop_name):
    ...
    def successful_login(self, urlrequest, log_in_data):
        ...
        # shapname = self.ShopName
        user_id = log_in_data['localId']
        idToken = log_in_data['idToken']
        # print(user_id, idToken,"--------sign up data")

        dummy_data = {"name":self.ShopName,"customers" :"","products":""}
                                
        post_request = requests.patch("https://as-udhar.firebaseio.com/" + user_id + ".json?auth=" + idToken, data= json.dumps(dummy_data))
